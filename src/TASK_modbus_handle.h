            ...
            { // RESET timer和风门时随时手动控制
                ...
                {                               // PID ON
                    if (pid_on_status == false) // 状态：mb.Hreg(PID_HREG) == 1 and pid_on_status == false
                    {
                        ...
                        Serial.printf("\n 4:PID_HREG:%d,pid_on_status:%d:init:%d", mb.Hreg(PID_ON_HREG), pid_on_status, init_status); // PID ON 当前状态是关
#endif
...
#if defined(DEBUG_MODE)
                        Serial.printf("\n 6:PID_HREG:%d,pid_on_status:%d:init:%d", mb.Hreg(PID_ON_HREG), pid_on_status, init_status);
#endif
                ...
                else // PID OFF
                {
...
#if defined(DEBUG_MODE)
                        Serial.printf("\n 1:PID_HREG:%d,pid_on_status:%d:init:%d", mb.Hreg(PID_ON_HREG), pid_on_status, init_status);
#endif
                    ...
                    {
#if defined(DEBUG_MODE)
                        Serial.printf("\n 2:PID_HREG:%d,pid_on_status:%d:init:%d", mb.Hreg(PID_ON_HREG), pid_on_status, init_status);
#endif
